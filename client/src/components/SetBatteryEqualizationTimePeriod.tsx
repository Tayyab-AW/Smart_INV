...
const SetBatteryEqualizationTimePeriod = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const res = await setInverterParameters("PBEQP", source);
  ...
  return (
    <div className="bg-[grey] flex flex-col mt-[50px] w-screen h-auto p-[20px] rounded-md">
      <span style={{ fontSize: "24px" }}>Set Equalization Period:</span>
      <form onSubmit={handleSubmit}>
        <input
          type="text"
          placeholder="Input Number of Days from 0 to 90"
          value={source}
...
export default SetBatteryEqualizationTimePeriod;
