...
const SetInverterChargerPriority = () => {
  ...
  const handleCommandChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const frequencyValue =
      e.target.value === "Solar First"
        ? "01"
        : e.target.value === "Solar and Utility"
        ? "02"
        : e.target.value === "Solar Only"
        ? "03"
        : "";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const res = await setInverterParameters("PCP", source);
  ...
  return (
    <div className="bg-[grey] flex flex-col mt-[50px] w-screen h-auto p-[20px] rounded-md">
      <span style={{ fontSize: "24px" }}>Set Inverter Charger Priority:</span>
...
export default SetInverterChargerPriority;
